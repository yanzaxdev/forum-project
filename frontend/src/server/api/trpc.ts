...
const t = initTRPC.context<typeof createTRPCContext>().create({
  ...
  errorFormatter({shape, error}) {
    return {
      ...
      data: {
        ...shape.data,
        zodError: error.cause instanceof ZodError ? error.cause.flatten() :
                                                    null,
      },
    };
  },
...
/**
 * Middleware for timing procedure execution and adding an artificial delay in
 * development.
 *
 * You can remove this if you don't like it, but it can help catch unwanted
 * waterfalls by simulating network latency that would occur in production but
